...
class UserRequests {
    ...
    suspend fun login(
        ...
        return try {
            ...
            Result.Error("Введены неверные данные")
        }
    }
    ...
    suspend fun loadUserData(): Result<Map<String, Any>> {
        return try {
            ...
            if (response.result == "success") {
                Result.Success(
                    mapOf<String, Any>(
                        "username" to if (response.username.toString() == "") "Имя не задано" else response.username.toString(),
                        "login" to response.login.toString(),
                        "createdQuizzes" to (response.created_quizes as? List<Quiz>
                            ?: mutableListOf<Quiz>()),
                        "doneQuizzes" to (response.done_quizes as? List<Quiz>
                            ?: mutableListOf<Quiz>())
                    )
                )
            } else {
            ...
            Result.Error("Ошибка загрузки данных пользователя")
        }
    }
